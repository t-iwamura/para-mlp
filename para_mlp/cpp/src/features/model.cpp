...
(const vector3d& dis_array, 
 ...
 const struct feature_params& fp){
    ...
    n_type = fp.n_type, force = fp.force, 
        model_type = fp.model_type, maxp = fp.maxp;
    modelp = ModelParams(fp);
...
(const vector3d& dis_array, const vector4d& diff_array,
 const vector3i& atom2_array, const struct feature_params& fp){
    ...
    for (int atom1 = 0; atom1 < n_atom; ++atom1){
        ...
        else local.pair_d(dis_array[atom1], diff_array[atom1], 
            atom2_array[atom1], de, dfx, dfy, dfz, ds);
 ...
 const vector3i& atom2_array, const struct feature_params& fp){
    ...
    for (int atom1 = 0; atom1 < n_atom; ++atom1){
        ...
        if (force == false) {
            ...
            local.gtinv_d
                (dis_array[atom1], diff_array[atom1], 
                 atom2_array[atom1], de, dfx, dfy, dfz, ds);
        }
        model_common(de, dfx, dfy, dfz, ds, types[atom1]);
    }
...
void Model::model_common
(const vector1d& de, const vector2d& dfx, const vector2d& dfy, 
 const vector2d& dfz, const vector2d& ds, const int& type1){
...
void Model::model_linear
(const vector1d& de, const vector2d& dfx, const vector2d& dfy, 
 const vector2d& dfz, const vector2d& ds, int& col){
