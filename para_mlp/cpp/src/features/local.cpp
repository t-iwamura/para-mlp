...
Local::Local
(const int& n_atom_i, const int& atom1_i, const int& type1_i,
 const struct feature_params& fp_i, const ModelParams& modelp_i){
...
vector1d Local::pair(const vector2d& dis_a){
    ...
    for (const auto& tc: modelp.get_type_comb_pair()){
        if (tc[type1].size() > 0){
            ...
            for (int j = 0; j < dis_a[type2].size(); ++j){
                dis = dis_a[type2][j];
                get_fn(dis, fp, fn);
...
(const vector2d& dis_a, const vector3d& diff_a, const vector2i& atom2_a,
 vector1d& an, vector2d& an_dfx, vector2d& an_dfy, 
 vector2d& an_dfz, vector2d& an_ds){
...
void Local::gtinv_d
(const vector2d& dis_a, const vector3d& diff_a, const vector2i& atom2_a,
 vector1d& dn, vector2d& dn_dfx, vector2d& dn_dfy, vector2d& dn_dfz, 
 vector2d& dn_ds){

    const vector2i &lm_info = get_lm_info(fp.maxl);
    vector3dc anlm;
    vector4dc anlm_dfx, anlm_dfy, anlm_dfz, anlm_ds;
    compute_anlm_d(dis_a, diff_a, atom2_a, lm_info, 
            anlm, anlm_dfx, anlm_dfy, anlm_dfz, anlm_ds);
...
vector3dc Local::compute_anlm
(const vector2d& dis_a, const vector3d& diff_a, const vector2i& lm_info){
    ...
    for (int type2 = 0; type2 < n_type; ++type2){
        for (int j = 0; j < dis_a[type2].size(); ++j){
            dis = dis_a[type2][j];
            const vector1d &sph = cartesian_to_spherical(diff_a[type2][j]);
            get_fn(dis, fp, fn);
...
(const vector2d& dis_a, const vector3d& diff_a, 
 const vector2i& atom2_a, const vector2i& lm_info, 
 vector3dc& anlm, vector4dc& anlm_dfx, vector4dc& anlm_dfy, 
 vector4dc& anlm_dfz, vector4dc& anlm_ds){
    ...
    double delx,dely,delz,dis,costheta,sintheta,cosphi,sinphi,coeff,cc;
    dc f1,ylm_dphi,d0,d1,d2,term1,term2,valx,valy,valz;
    int atom2,m,lm1,lm2;
        ...
        for (int j = 0; j < dis_a[type2].size(); ++j){
            ...
            for (int lm = 0; lm < n_lm; ++lm) {
                ...
                for (int n = 0; n < n_fn; ++n) {
                    ...
                    f1 = fn_d[n] * ylm[lm];
                    valx = (f1 * delx + fn[n] * d0) / dis;
                    valy = (f1 * dely + fn[n] * d1) / dis;
                    valz = (f1 * delz + fn[n] * d2) / dis;
