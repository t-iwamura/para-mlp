...
(const vector3d& axis, 
 ...
 const bool& print_memory){
    ...
    struct feature_params fp = {
        n_type,force,params,cutoff,pair_type,des_type,model_type,
        maxp,maxl,lm_array,l_comb,lm_coeffs
    };

    std::vector<bool> force_st;
    vector1i xf_begin, xs_begin;
    int n_data;
    set_index(n_st_dataset, force_dataset, n_atoms_all, 
        xf_begin, xs_begin, force_st, n_data);

    Neighbor neigh(axis[0], positions_c[0], types[0], fp.n_type, fp.cutoff);
    Model mod(neigh.get_dis_array(), neigh.get_diff_array(),
            neigh.get_atom2_array(), types[0], fp);
    ...
    for (int i = 0; i < n_st; ++i){
        ...
        Neighbor neigh
            (axis[i], positions_c[i], types[i], fp1.n_type, fp1.cutoff);
        Model mod(neigh.get_dis_array(), neigh.get_diff_array(),
                neigh.get_atom2_array(), types[i], fp1);
...
void ModelPy::set_index
(const std::vector<int>& n_data_dataset, 
 const std::vector<bool>& force_dataset,
 const std::vector<int>& n_atoms_st,
 std::vector<int>& xf_begin, std::vector<int>& xs_begin,
 std::vector<bool>& force, int& n_row){
...
Eigen::MatrixXd& ModelPy::get_x(){ return x_all; }
const vector1i& ModelPy::get_fbegin() const{ return xf_begin_dataset; }
const vector1i& ModelPy::get_sbegin() const{ return xs_begin_dataset; }
