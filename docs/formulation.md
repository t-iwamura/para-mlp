...
Let $H(\mathbf{R}, \mathbf{S})$ be the total energy of given atomic and spin configuration.

\begin{align*}
    Z = \sum_{\mathbf{S}} \exp(-\beta H(\mathbf{R}, \mathbf{S}))
\end{align*}
...
:label: eq_1st_expansion
    Z &= \sum_{\mathbf{S}} \left( 1 - \beta H(\mathbf{R}, \mathbf{S}) +
        \frac{\beta^{2} H^{2}(\mathbf{R}, \mathbf{S})}{2!} + \cdots +
        \frac{(- \beta)^{n}H^{n}(\mathbf{R}, \mathbf{S})}{n!} + \cdots + \right) \\
    &= Z_{0}\left( 1 - \beta <H>_{0} + \frac{\beta^{2} <H^{2}>_{0}}{2} + \cdots \right)
```

$Z_{0}$ and $<H^{n}>_{0}$ are defined as

\begin{gather*}
    Z_{0} := \sum_{\mathbf{S}} 1 \\
    <H^{n}>_{0} := \frac{\sum_{\mathbf{S}} H^{n}(\mathbf{R}, \mathbf{S})}{Z_{0}}
\end{gather*}
...
\begin{align*}
    \ln Z &= \ln Z_{0} + \left( - \beta <H>_{0} + \frac{\beta^{2} <H^{2}>_{0}}{2} + \cdots \right)
        - \frac{1}{2} \left( - \beta <H>_{0} + \frac{\beta^{2} <H^{2}>_{0}}{2} + \cdots \right)^{2}
        + \cdots \\
    &= \ln Z_{0} - \beta <H>_{0} + \frac{\beta^{2}}{2} \left( <H^{2}>_{0} - <H>_{0}^{2} \right)
        + \mathcal{O}(\beta^{3}) \\
    &= \ln Z_{0} - \beta <H>_{0} + \mathcal{O}(\beta^{2})
\end{align*}
...
\begin{align*}
    F &= - \beta^{-1} \ln Z \\
    &= - \beta^{-1} \ln Z_{0} + <H>_{0} + \mathcal{O}(\beta)
\end{align*}
...
We denote the total energy of given atomic configuration $\mathbf{R}$ and
spin configuration $\mathbf{S}$ by $H(\mathbf{R}, \mathbf{S})$.

\begin{align*}
    H(\mathbf{R}, \mathbf{S}) &= \sum_{n} \sum_{t} w_{n, t} \left(\sum_{i} d_{n, t}^{(i)} \right)\\
...
\begin{align*}
    \mathcal{N}_{i, t} := \left\{ j \mid j = 1, 2, \cdots, N,\,r_{ij} \le r_{\mathrm{cutoff}}, \{s_{i}, s_{j}\} = t \right\}
\end{align*}
...
Let $E_{\mathbf{R}, \mathbf{S}, \{i, j\}}$ be the term in $H(\mathbf{R}, \mathbf{S})$ that only the atom $i$ and the atom $j$ involve in. Then,

\begin{gather*}
    H(\mathbf{R}, \mathbf{S}) = \sum_{\{i, j\} \in \mathcal{D}} E_{\mathbf{R}, \mathbf{S}, \{i, j\}} \\
    E_{\mathbf{R}, \mathbf{S}, \{i, j\}} = \sum_{n} w_{n,\{s_{i},s_{j}\}} \left( f_{n}(r_{ij}) + f_{n}(r_{ji}) \right)
\end{gather*}

Therefore, $<H>_{0}$ is written as

\begin{align*}
    <H>_{0} &= \sum_{\mathbf{S}} H(\mathbf{R}, \mathbf{S}) \\
    &= \sum_{\{i, j\} \in \mathcal{D}_{2}} \left( \sum_{\mathbf{S}} E_{\mathbf{R}, \mathbf{S}, \{i, j\}} \right) \\
    &= \sum_{\{i, j\} \in \mathcal{D}_{2}} \sum_{n} \left( \sum_{\mathbf{S}} w_{n, \{s_{i}, s_{j}\}}(f_{n}(r_{ij})
        + f_{n}(r_{ji}))) \right) \\
...
\begin{align*}
    H(\mathbf{R}, \mathbf{S}) &= \sum_{n} \sum_{l=0}^{l^{(2)}_{\mathrm{max}}} \sum_{\mathbf{\tau} \in \mathcal{T}} w_{nl, \mathbf{\tau}}
...
Let $E_{\mathbf{R}, \mathbf{S}, \Delta}$ be the term in $H(\mathbf{R}, \mathbf{S})$ that only the atoms
in $\Delta \in \mathcal{D}_{n}(n = 2, 3)$ involve in. Then

\begin{align*}
    H(\mathbf{R}, \mathbf{S}) &= \sum_{\{i, j\} \in \mathcal{D}_{2}} E_{\mathbf{R}, \mathbf{S}, \{i, j\}}
        + \sum_{\{i, j, k\} \in \mathcal{D}_{3}} E_{\mathbf{R}, \mathbf{S}, \{i, j, k\}} \\
\end{align*}

Here, each term is defined as follows.
\begin{align*}
    E_{\mathbf{R}, \mathbf{S}, \{i, j\}} & := \sum_{n} \sum_{l=0}^{l^{(2)}_{\mathrm{max}}}
        \sum_{m_{1}=-l}^{l} \sum_{m_{2}=-l}^{l}
        \left[ G^{(i)}_{nlm_{1}m_{2}}(j, j)
        + G^{(j)}_{nlm_{1}m_{2}}(i, i) \right] \\
    E_{\mathbf{R}, \mathbf{S}, \{i, j, k\}} & := \sum_{n} \sum_{l=0}^{l^{(2)}_{\mathrm{max}}}
...
\begin{align*}
    <H>_{0} &= \sum_{\mathbf{S}} H(\mathbf{R}, \mathbf{S}) \\
        &= \sum_{\{i,j\} \in \mathcal{D}_{2}} \left( \sum_{\mathbf{S}} E_{\mathbf{R},\mathbf{S},\{i,j\}} \right)
            + \sum_{\{i,j,k\} \in \mathcal{D}_{3}} \left( \sum_{\mathbf{S}} E_{\mathbf{R},\mathbf{S},\{i,j,k\}} \right) \\
...
Then, $H(\mathbf{R}, \mathbf{S})$ is written as

\begin{align*}
    H(\mathbf{R}, \mathbf{S}) &= \sum_{n} \sum_{\mathbf{k} \in \mathcal{L}}
...
\begin{align*}
    H(\mathbf{R}, \mathbf{S}) &= \sum_{\{i, j\} \in \mathcal{D}_{2}} E_{\mathbf{R}, \mathbf{S}, \{i, j\}}
        + \sum_{\{i, j, k\} \in \mathcal{D}_{3}} E_{\mathbf{R}, \mathbf{S}, \{i, j, k\}}
        + \sum_{\{i, j, k, l\} \in \mathcal{D}_{4}} E_{\mathbf{R}, \mathbf{S}, \{i, j, k, l\}} \\
\end{align*}

Here, each term is defined as follows.
\begin{align*}
    E_{\mathbf{R}, \mathbf{S}, \{i, j\}} & := \sum_{n} \sum_{\mathbf{k}=(l_{1}, l_{2}, l_{3}) \in \mathcal{L}}
    ...
    E_{\mathbf{R}, \mathbf{S}, \{i, j, k\}} & := \sum_{n} \sum_{\mathbf{k}=(l_{1}, l_{2}, l_{3}) \in \mathcal{L}}
        \sum_{m_{1}=-l_{1}}^{l_{1}} \sum_{m_{2}=-l_{2}}^{l_{2}}
        \sum_{m_{3}=-l_{3}}^{l_{3}} \{
    ...
    E_{\mathbf{R}, \mathbf{S}, \{i, j, k, l\}} & := \sum_{n}
...
\begin{align*}
    <H>_{0} &= \sum_{\mathbf{S}} H(\mathbf{R}, \mathbf{S}) \\
        &= \sum_{\{i,j\} \in \mathcal{D}_{2}} \left( \sum_{\mathbf{S}} E_{\mathbf{R},\mathbf{S},\{i,j\}} \right)
            + \sum_{\{i,j,k\} \in \mathcal{D}_{3}} \left( \sum_{\mathbf{S}} E_{\mathbf{R},\mathbf{S},\{i,j,k\}} \right)
            + \sum_{\{i,j,k,l\} \in \mathcal{D}_{4}} \left( \sum_{\mathbf{S}} E_{\mathbf{R},\mathbf{S},\{i,j,k,l\}} \right)
\end{align*}
